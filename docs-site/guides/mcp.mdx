...
MCP is **off by default**. In the ComfyTV sidebar open **Settings → Agent & MCP** and turn on **Enable MCP server**. (The embedded [Bot](/guides/bot) additionally requires its own switch.)
...
- [ComfyTV Bot](/guides/bot) — the embedded chat agent built on these same tools
- [Custom workflows](/guides/custom-workflows) — what bindings are, hand-edited
- [Sidebar](/guides/sidebar) — the Settings panel with the MCP switch
